...
Simple Agent Manager is a node-based visual AI agent builder. The React app lets users compose graphs on a canvas, resolve them into serializable `AgentConfig` JSON, and run them through a local Express + WebSocket backend. See README.md for full overview.
...
4. If you add a brand-new node type that has no entry in `docs/concepts/_manifest.json` yet, create its concept doc from `docs/concepts/_template.md` and add it to the manifest first.
