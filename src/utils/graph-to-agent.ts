...
export function resolveAgentConfig(
  ...
  // --- Sandbox / Compute ---
  // At most one sandbox node binds to an agent; it governs the single shared
  // execution environment for code-running tools, so the first connected node
  // wins and resolves to a single optional value (or null), like reflection.
  ...
  const sandbox: ResolvedSandboxConfig | null =
    sandboxNode && sandboxNode.data.type === 'sandbox'
      ? {
          ...
          readOnlyRoot: sandboxNode.data.readOnlyRoot,
          ...
          blockMessage: sandboxNode.data.blockMessage,
        }
      : null;
