...
export function resolveAgentConfig(
  ...
  // At most one A2A node binds to an agent; it configures both the exposed
  // server and the set of remote delegates, so the first connected node wins
  ...
  const a2a: ResolvedA2AConfig | null =
    a2aNode && a2aNode.data.type === 'a2a'
      ? {
          ...
          exposeServer: a2aNode.data.exposeServer,
          ...
          maxDelegationDepth: a2aNode.data.maxDelegationDepth,
