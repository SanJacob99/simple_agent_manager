...
export function resolveAgentConfig(
  ...
  // At most one A2A node binds to an agent — it owns the single published
  // server surface / delegate registry — so the first connected node wins and
  // resolves to a single optional value (or null), mirroring reflection.
  ...
  const a2a: ResolvedA2AConfig | null =
    a2aNode && a2aNode.data.type === 'a2a'
      ? {
          ...
          inboundTokenEnv: a2aNode.data.inboundTokenEnv,
          remotes: a2aNode.data.remotes.map((r) => ({
            id: r.id,
            name: r.name,
            cardUrl: r.cardUrl,
            authTokenEnv: r.authTokenEnv,
            enabled: r.enabled,
          })),
