...
export function resolveAgentConfig(
  ...
  // card and delegate registry), so the first connected node wins and resolves
  // to a single optional value, mirroring reflection / structured output.
  ...
  const a2a: ResolvedA2AConfig | null =
    a2aNode && a2aNode.data.type === 'a2a'
      ? {
          ...
          inboundAuthScheme: a2aNode.data.inboundAuthScheme,
          ...
          exposeAsTools: a2aNode.data.exposeAsTools,
          taskTimeoutMs: a2aNode.data.taskTimeoutMs,
          maxDelegationsPerRun: a2aNode.data.maxDelegationsPerRun,
        }
      : null;
