...
- Engine: `server/a2a/a2a-engine.ts` (dependency-free agent-card builder + remote-card
  validator, delegate-tool resolver, JSON-RPC `message/send` framing, and task-result
  parsing over the A2A task lifecycle)
- Doc: `docs/concepts/a2a-node.md`
- **Remaining:** mount the A2A server router in `server/index.ts` (serve
  `buildAgentCard(...)` at `<exposePath>/.well-known/agent-card.json` and answer
  `message/send` / `message/stream` by driving a headless run), register the
  descriptors from `resolveDelegateTools(...)` as tools in
  `server/tools/tool-factory.ts` (fetch + validate the remote card, POST the
  request with `buildAuthHeaders(...)`, poll while `!isTerminalTaskState(state)`),
  and emit `a2a:task` events from `server/agents/run-coordinator.ts`.
...
7. **A2A** (#8) is scaffolded — next is mounting the server router, registering the
   delegate tools, and emitting `a2a:task` events (see the item's *Remaining*). It
   is the cross-framework interop counterpart to the in-process `agentComm` bus.
