...
tools. The `a2a` node exposes this agent as an A2A server (publishes an agent
card, accepts remote tasks) and/or registers remote A2A agents as callable
delegates.
...
- Engine: `server/a2a/a2a-engine.ts` (dependency-free agent-card builder, task
  state machine, `message/send` validation, and delegate-tool derivation)
- Doc: `docs/concepts/a2a-node.md`
- **Remaining:** mount an Express router under `server/a2a/` (serve the agent
  card + JSON-RPC task endpoints, bridge `runtime.prompt()` to the task state
  machine, stream status updates), register `resolveDelegateTools(...)` output
  through `server/tools/tool-factory.ts`, and resolve each remote's `authRef`
  from the credential store at call time.
...
7. **A2A** (#8) is scaffolded — next is mounting the `server/a2a/` router
   (agent-card + task endpoints, delegate-tool registration, credential
   resolution). **Sandbox** (#10) remains the next design wave for execution
   safety.
