...
delegates, across JSON-RPC / gRPC / HTTP+JSON transports.

- Node: `a2a` (`src/types/nodes.ts#A2ANodeData`)
- Resolved: `AgentConfig.a2a` (`shared/agent-config.ts#ResolvedA2AConfig`, a list
  like evals/budgets — a graph can carry a server node and client nodes)
- Engine: `server/a2a/a2a-engine.ts` (dependency-free, side-effect-free: agent-card
  builder, well-known path, config validation, transport selection, and delegate
  tool descriptors; `authRef` stays a reference, never a secret)
- Doc: `docs/concepts/a2a-node.md`
- **Remaining:** add the network layer — an Express router that serves the card at
  `/.well-known/agent-card.json` and terminates the A2A task/message endpoint, plus
  a client that performs the JSON-RPC / gRPC / HTTP+JSON call to a remote agent —
  register `buildDelegateTools()` output through `server/tools/tool-factory.ts`, and
  resolve each `authRef` credential out of band. Complements `subAgent` (in-tree)
  and `agentComm` (in-process) with a network-standard boundary.
...
7. **A2A** (#8) is scaffolded — next is the network layer (serve the agent card,
   terminate the task endpoint, perform the remote transport call) and registering
   its delegate tools through the tool factory. **Sandbox** (#10) is the remaining
   design-wave item — execution safety for code-running agents.
