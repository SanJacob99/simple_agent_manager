...
delegates. Like structured output / reflection, at most one A2A node binds to an
agent (it owns the single card identity), resolving to one optional value.

- Node: `a2a` (`src/types/nodes.ts#A2ANodeData`)
- Resolved: `AgentConfig.a2a` (`shared/agent-config.ts#ResolvedA2AConfig`)
- Engine: `server/a2a/a2a-engine.ts` (dependency-free agent-card builder, task
  state machine, inbound message/auth validation, delegate-tool synthesis, and
  remote-card parsing)
- Doc: `docs/concepts/a2a-node.md`
- **Remaining:** add an Express router under `server/a2a/` (serve the card at
  `cardPath`, accept `message/send` + `message/stream`, drive the task-state
  machine around `runtime.prompt()`), register `buildDelegateTools` output
  through `server/tools/tool-factory.ts`, and resolve the inbound token from
  `authTokenEnvVar` at request time.
...
7. **A2A** (#8) is scaffolded — next is the `server/a2a/` router (serve the
   card, accept `message/send` / `message/stream`) and registering the delegate
   tools through the tool factory. **Sandbox** (#10) is the remaining design-wave
   item — execution safety for code-running agents.
