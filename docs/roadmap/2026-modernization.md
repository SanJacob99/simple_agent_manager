...
- Engine: `server/a2a/a2a-engine.ts` (dependency-free Agent Card builder,
  well-known card URL, inbound message-envelope validation + text extraction,
  the task-state machine, and remote-delegate lookup)
- Doc: `docs/concepts/a2a-node.md`
- **Remaining:** add an Express route that serves the card and handles
  `message/send` + `message/stream` (driving a headless run per task through
  `server/agents/run-coordinator.ts`), build the outbound delegate client that
  fetches a peer card and posts tasks, and resolve `authScheme` against real
  credentials.
...
7. **A2A** (#8) is scaffolded — next is the HTTP surface: an Express route that
   serves the Agent Card and handles `message/send` / `message/stream`, plus the
   outbound delegate client. **Sandbox** (#10) remains the next design item for
   execution safety.
