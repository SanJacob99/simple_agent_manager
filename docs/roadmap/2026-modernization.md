...
- Engine: `server/a2a/a2a-engine.ts` (dependency-free agent-card builder,
  JSON-RPC `message/send` envelope construction, task-result parsing, remote-agent
  tool selection, and the `fail`/`warn`/`ignore` remote-error policy)
- Doc: `docs/concepts/a2a-node.md`
- **Remaining:** mount the card route (`/.well-known/agent-card.json`) and a
  `message/send` handler in the server when `exposeAsServer`, register
  `a2a__<id>` delegate tools for `remoteAgents` in `server/tools/tool-factory.ts`,
  wire inbound auth, and emit `a2a:remote_error`.
...
7. **A2A** (#8) is scaffolded — next is mounting the card route + `message/send`
   handler in the server and registering `a2a__<id>` delegate tools in the tool
   factory (see its *Remaining*). It brings cross-framework interop to the builder.
8. **Sandbox** (#10) is the remaining design item — execution safety for the
   `exec` / `code_execution` tools; it is the next node to scaffold.
