...
- Engine: `server/a2a/a2a-engine.ts` (dependency-free: agent-card assembly and
  validation, task/message envelope construction, task-state normalization,
  remote-response parsing, and delegate selection; ids/timestamps injected by the
  caller so it stays deterministic under test)
- Doc: `docs/concepts/a2a-node.md`
- **Remaining:** implement the HTTP surface in `server/a2a/` — serve the card at
  `/.well-known/agent-card.json`, a JSON-RPC/REST task endpoint, SSE streaming,
  and the fetch client that dispatches to remotes — and wire task delegation into
  `server/agents/run-coordinator.ts` (expose registered remotes as callable
  delegates, apply `onRemoteError`, emit `a2a:task` / `a2a:remote_error` events).
...
7. **A2A** (#8) is scaffolded — next is the `server/a2a/` HTTP surface (serve the
   card, task endpoint, SSE streaming, remote fetch client) and run-coordinator
   delegation wiring. **Sandbox** (#10) is the next design wave for execution safety.
