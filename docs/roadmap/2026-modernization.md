...
- Engine: `server/a2a/a2a-engine.ts` (dependency-free: Agent Card assembly, the
  well-known URL conventions, remote-delegate validation, JSON-RPC `message/send`
  construction, and Task/Message response parsing)
- Doc: `docs/concepts/a2a-node.md`
- **Remaining:** serve the Agent Card + a `message/send` handler under `basePath`
  from the Express app, dispatch delegated tasks to `remotes` (honouring
  `taskTimeoutMs` / `maxConcurrentTasks`) from `server/agents/run-coordinator.ts`,
  and resolve `credentialEnvVar` secrets for authed remotes. Pairs with the
  Telemetry node (#1) for cross-agent task spans.
...
7. **A2A** (#8) is scaffolded — next is serving the Agent Card + `message/send`
   handler from the Express app and dispatching delegated tasks from the
   run-coordinator. **Sandbox** (#10) remains the next design wave for execution
   safety.
