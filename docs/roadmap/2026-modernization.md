...
- Engine: `server/a2a/a2a-engine.ts` (dependency-free agent-card assembly,
  `a2a_<slug>` delegate-tool naming with collision numbering, the JSON-RPC
  `message/send` envelope, tolerant task-result extraction, and config
  validation)
- Doc: `docs/concepts/a2a-node.md`
- **Remaining:** serve the agent card + task endpoint from `server/a2a/` routes
  (gate on `isServerActive`, authenticate per `serverAuthScheme`), and wire
  `buildRemoteDelegateTools` into `server/tools/tool-factory.ts` so each enabled
  remote becomes a tool that fetches the card, resolves the endpoint, POSTs the
  `message/send` envelope, and returns `parseTaskResult`. Pairs with the
  Sandbox node (#10) for isolating remote-task side effects.
...
7. **A2A** (#8) is scaffolded — next is serving the agent card + task endpoint
   from `server/a2a/` routes and wiring `buildRemoteDelegateTools` into the tool
   factory so remotes become callable delegates.
8. **Sandbox** (#10) is the remaining design-wave item — execution safety for
   code-running agents; pairs with A2A for isolating remote-task side effects.
