...
talk to agents built on *other* frameworks. The Agent-to-Agent (A2A) protocol
(agent cards, task/message envelopes, streaming updates over JSON-RPC/gRPC/REST,
now stewarded by the Linux Foundation) is becoming the lingua franca for
cross-framework agent interop, much as MCP standardized tools. The `a2a` node
exposes this agent as an A2A server (publish an agent card, accept remote tasks)
and/or registers remote A2A agents as callable delegates.
...
- Engine: `server/a2a/a2a-engine.ts` (dependency-free Agent Card build/validate,
  the task-state machine, JSON-RPC `message/send` shaping + task-result parsing,
  and remote-agent → delegate-tool specs)
- Doc: `docs/concepts/a2a-node.md`
- **Remaining:** serve `buildAgentCard(...)` at the well-known path from
  `server/a2a/` (JSON-RPC handler + optional SSE streaming + task store), bind
  `remoteAgentToolSpec(...)` delegates into the tool factory, and forward a
  delegate call over the transport from `server/agents/run-coordinator.ts`.
...
7. **A2A** (#8) is scaffolded — next is standing up the `server/a2a/` transport
   (serve the Agent Card, run the task lifecycle, bind remote delegates as tools).
