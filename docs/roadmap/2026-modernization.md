...
- Resolved: `AgentConfig.a2a` (`shared/agent-config.ts#ResolvedA2AConfig`, split
  into `server` / `client` sub-configs)
- Engine: `server/a2a/a2a-engine.ts` (dependency-free, transport-agnostic:
  agent-card builder, JSON-RPC 2.0 request parsing, the task-state machine,
  incoming-message parsing, delegate selection, and envelope helpers)
- Doc: `docs/concepts/a2a-node.md`
- **Remaining:** mount an Express/SSE A2A router under `server/a2a/` at
  `server.path`, drive a headless ephemeral run per accepted task, register each
  remote as a delegate tool honoring the `onError` policy, and emit `a2a:task`
  lifecycle events.
...
7. **A2A** (#8) is scaffolded — next is mounting the A2A router under
   `server/a2a/` (headless run per task, remotes as delegate tools, task events).
   **Sandbox** (#10) is the next design wave for execution safety.
