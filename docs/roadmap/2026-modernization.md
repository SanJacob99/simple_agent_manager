...
protocol (agent cards, JSON-RPC task/message envelopes, streaming updates) is
becoming the lingua franca for cross-framework agent interop, much as MCP
standardized tools. The `a2a` node exposes this agent as an A2A server (publish
an agent card, accept remote tasks) and/or registers remote A2A agents as
callable delegates.

- Node: `a2a` (`src/types/nodes.ts#A2ANodeData`)
- Resolved: `AgentConfig.a2a` (`shared/agent-config.ts#ResolvedA2AConfig`)
- Engine: `server/a2a/a2a-engine.ts` (dependency-free agent-card builder/validator,
  security-scheme mapping, JSON-RPC `message/send` framing, task-response
  normalization, and delegate resolution; generates no ids/timestamps so it stays
  deterministic)
- Doc: `docs/concepts/a2a-node.md`
- **Remaining:** serve the card route (`isServerEnabled` + `buildAgentCard`) from
  the Express layer, register the `resolveDelegates` tools in
  `server/tools/tool-factory.ts`, and pump the JSON-RPC transport
  (`buildMessageSendRequest` / `parseTaskResponse`) with real HTTP + auth in
  `server/a2a/`.
...
7. **A2A** (#8) is scaffolded — next is serving the agent-card route, registering
   the resolved delegates as tools, and pumping the JSON-RPC transport with real
   HTTP + auth (see its *Remaining*). **Sandbox** (#10) is the next design wave for
   execution safety.
