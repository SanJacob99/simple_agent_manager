...
2. **Runtime `os=` rewrite.** The client builds the `runtime` section from the browser's `navigator.platform`, but the agent actually executes server-side, so `rewriteRuntimeOs()` replaces the `os=<value>` field (in both the `runtime` section and the assembled string) with the backend's real `process.platform` before the prompt is sent to the model.
