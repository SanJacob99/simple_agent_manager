...
2. **Runtime OS rewrite.** The `os=` field in the `runtime` section (and the assembled string) is rewritten from whatever the client sent — the frontend fills it from the browser's `navigator.platform` — to the backend's actual `process.platform`, since the agent executes server-side, not in the browser. Token estimates are recomputed when the rewrite changes the section's content.
