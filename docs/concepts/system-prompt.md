...
2. **Runtime OS rewrite.** The `runtime` section's `os=` value, built client-side from the browser's `navigator.platform`, is rewritten (`rewriteRuntimeOs`) to the backend's real `process.platform` in the same pass as the skills-root substitution.
