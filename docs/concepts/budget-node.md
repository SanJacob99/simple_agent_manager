...
`src/utils/graph-to-agent.ts` resolves each connected Budget node into a `ResolvedBudgetConfig` entry on `AgentConfig.budgets` (`shared/agent-config.ts`). The field is typed optional, but resolution always assigns an array — agents without a Budget node have `budgets === []` and the runtime enforces no ceilings.
