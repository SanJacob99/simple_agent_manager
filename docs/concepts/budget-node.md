...
`src/utils/graph-to-agent.ts` resolves each connected Budget node into a `ResolvedBudgetConfig` entry on `AgentConfig.budgets` (`shared/agent-config.ts`). `budgets` is typed optional for backward compatibility, but `resolveAgentConfig()` always assigns it — agents without a Budget node get `budgets === []`, not `undefined`, and the runtime enforces no ceilings for an empty list.
