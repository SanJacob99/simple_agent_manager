...
`src/utils/graph-to-agent.ts` resolves each connected Budget node into a `ResolvedBudgetConfig` entry on `AgentConfig.budgets` (`shared/agent-config.ts`). The field is optional only for backward compatibility with older fixtures — the resolver always produces an array, so agents without a Budget node have `budgets === []`, and the runtime enforces no ceilings.
