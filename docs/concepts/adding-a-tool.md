...
Before the `ToolModule` migration, adding a tool touched 5–9 files: the implementation plus edits to `TOOL_GROUPS`, `ALL_TOOL_NAMES`, and `IMPLEMENTED_TOOL_NAMES` in [shared/resolve-tool-names.ts](../../shared/resolve-tool-names.ts), a branch in `createAgentTools()` in [server/tools/tool-factory.ts](../../server/tools/tool-factory.ts), a field on `ToolFactoryContext`, a field on `AgentConfig`, plus the UI wiring. All built-in tools — including `calculator`, which now has a `calculator.module.ts` — live in `ToolModule`s and are served from the registry, which takes precedence over the factory's `TOOL_CREATORS` map. That legacy stub map still exists in the factory as a fallback but is unreachable in practice since every name it lists is also registry-served. Do not add new tools through the legacy path — the effort is the same as writing a module, the ergonomics are worse, and the factory branch is on its way out.
