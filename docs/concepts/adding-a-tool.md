...
- `state-mutating` — `write_file`, `edit_file`, `image_generate` (writes a file to disk). Reversible or scoped side effects. (`send_message` is listed in `shared/resolve-tool-names.ts`'s tool groups but has no `ToolModule` implementation yet, so it isn't a real example of this classification today.)
...
- **Aliases.** Two exist today: `bash` → `exec` and `code_interpreter` → `code_execution` (`TOOL_ALIASES` in `tool-registry.ts`, mirrored by `TOOL_NAME_ALIASES` in `shared/resolve-tool-names.ts`). New tools should pick a unique top-level name rather than declaring an alias.

---

## Legacy path (historical)

Before the `ToolModule` migration, adding a tool touched 5–9 files: the implementation plus edits to `TOOL_GROUPS`, `ALL_TOOL_NAMES`, and `IMPLEMENTED_TOOL_NAMES` in [shared/resolve-tool-names.ts](../../shared/resolve-tool-names.ts), a branch in `createAgentTools()` in [server/tools/tool-factory.ts](../../server/tools/tool-factory.ts), a field on `ToolFactoryContext`, a field on `AgentConfig`, plus the UI wiring. All built-in tools have since migrated to `ToolModule`s — including `calculator`, which now has both a real module ([calculator.module.ts](../../server/tools/builtins/calculator/calculator.module.ts)) and its original implementation ([calculator.ts](../../server/tools/builtins/calculator/calculator.ts)). Because the registry check runs first in `createAgentTools()`, the legacy `TOOL_CREATORS` fallback is currently unreachable dead code — there is no tool left that only exists in the legacy path. Do not add new tools through the legacy path — the effort is the same as writing a module, the ergonomics are worse, and the factory branch is on its way out.
