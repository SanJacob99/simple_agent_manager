...
- **Aliases.** `bash` → `exec` and `code_interpreter` → `code_execution` are the only aliases today (`server/tools/tool-registry.ts`'s `TOOL_ALIASES`). New tools should pick a unique top-level name rather than declaring an alias.
