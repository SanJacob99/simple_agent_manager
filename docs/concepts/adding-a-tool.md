...
- **Aliases.** `TOOL_ALIASES` in `server/tools/tool-registry.ts` currently declares `bash` → `exec` and `code_interpreter` → `code_execution`. New tools should pick a unique top-level name rather than declaring another alias.
