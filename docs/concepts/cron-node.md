...
| `sessionMode` | `"persistent" \| "ephemeral"` | `"persistent"` | Declared intent for session continuity across ticks. **Not yet read by `CronScheduler`** — see Runtime Behavior. |
...
| `retentionDays` | `number` | `7` | Declared per-cron retention intent. **Not yet consumed anywhere** — see Runtime Behavior. |
...
`sessionMode` is not yet wired: `executeCronTick` in
[`CronScheduler`](../../server/scheduling/cron-scheduler.ts) always
dispatches with a fixed `cron:${cronNodeId}` session key regardless of
the field's value, so every cron node effectively behaves as
`persistent` today. Treat `sessionMode: 'ephemeral'` as a declared
intent that has no runtime effect yet.

`retentionDays` is likewise not consumed anywhere in the codebase today
— it is defined on the type, has a default, and round-trips through
`graph-to-agent.ts` into `AgentConfig`, but no storage or maintenance
code reads it. Storage-level retention/rotation is a separate,
already-implemented mechanism (`StorageEngine.cleanResetArchives()` and
`runMaintenance()` in `server/storage/storage-engine.ts`, exposed via
`POST /api/storage/maintenance`), but it is not driven by any
individual cron node's `retentionDays` value.
