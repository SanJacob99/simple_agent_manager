...
As with the [MCP node](mcp-node.md), the server-side MCP runtime that would actually spawn the process, register its tools, and emit `mcp:status` events is **not yet implemented**. Resolution into `AgentConfig.mcps` happens correctly, but nothing downstream consumes it yet — connecting a Connector Node currently has no runtime effect. The UI status-dot wiring in `agent-connection-store.ts` is shared with the MCP node but has no events to react to for connectors today.
