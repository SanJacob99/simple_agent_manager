...
2. Materialized into a `ResolvedMcpConfig`:
   ...
   - `autoConnect` = `true`.
3. Appended to the same `mcps[]` the MCP node populates. As with bare MCP nodes, the server-side MCP client (subprocess spawn, tool registration, `mcp:status` events) is **not yet implemented**. The resolved config is prepared and will be consumed once the MCP runtime layer is wired in.
