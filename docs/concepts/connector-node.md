...
2. Materialized into a `ResolvedMcpConfig`:
   ...
   - `autoConnect` = `true`.
3. Appended to the same `mcps[]` the MCP node populates. As with plain MCP nodes, the actual MCP client (subprocess spawn, tool registration, `mcp:status` emission) is **not yet implemented at runtime** — see [mcp-node.md](mcp-node.md#runtime-behavior) for what that path still needs.
