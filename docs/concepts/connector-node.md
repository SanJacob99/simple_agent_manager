...
Like the MCP node it shares `mcps[]` with, the actual MCP client (subprocess spawn, tool registration, `mcp:status` emission) is **not yet implemented at runtime** — see [MCP node § Runtime Behavior](mcp-node.md#runtime-behavior). The connector node has no live connection-status indicator yet.
