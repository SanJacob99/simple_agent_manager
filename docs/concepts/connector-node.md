...
As with the MCP node, the actual MCP client (subprocess spawn, tool registration, `mcp:status` events) is **not yet implemented at runtime** — no server-side MCP manager exists under `server/` today. See [mcp-node.md](mcp-node.md#runtime-behavior) for the planned shape.
