...
As with the MCP node (see `docs/concepts/mcp-node.md`), no server-side MCP client exists yet — there is nothing in `server/` that spawns the resolved `ResolvedMcpConfig` entries, registers their tools, or emits `mcp:status`. Resolution into `AgentConfig.mcps[]` is real and complete; actually launching a connector's MCP server and exposing its tools is not yet implemented at runtime.
