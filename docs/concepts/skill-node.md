...
3. Skills with empty `content` render as a bullet-list entry in the `## Skills` system prompt section. Skills with authored content (typically from the Tools Node's `toolSettings.*.skill` fields or from `SkillDefinition` entries on the Tools Node) render as full markdown guidance blocks below the bullet list.
