...
**tools** (see `docs/concepts/user-tools-guide.md` and `bin/commands/`). User-installed
...
Once each Q has a "decided / leaning / deferred" annotation, open a normal implementation chunk modeled on the tool-install path's chunk-by-chunk shape:
