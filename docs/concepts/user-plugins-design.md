...
**tools** (see [user-tools-guide.md](user-tools-guide.md)). User-installed
...
Once each Q has a "decided / leaning / deferred" annotation, open a normal implementation chunk modeled on the tool-install work in [user-tools-guide.md](user-tools-guide.md) with the same chunk-by-chunk shape:
