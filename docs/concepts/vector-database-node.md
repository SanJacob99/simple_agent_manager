...
Resolved by `src/utils/graph-to-agent.ts` into `AgentConfig.vectorDatabases: ResolvedVectorDatabaseConfig[]`. The `AgentRuntime` constructor (`server/runtime/agent-runtime.ts`) calls `createVectorTools(config, runtimeHints)` — the four tool modules in `server/runtime/vector-tools/` wrap `getOrCreateVectorEngine` from `server/runtime/vector-engine-registry.ts` to resolve a shared `VectorDatabaseEngine` instance per collection.
