...
Resolved by `src/utils/graph-to-agent.ts` into `AgentConfig.vectorDatabases: ResolvedVectorDatabaseConfig[]`. `AgentRuntime` calls `createVectorTools(config, runtime)` (`server/runtime/vector-tools/index.ts`), which resolves each tool's engine through `getOrCreateVectorEngine(config, label, runtime)` (`server/runtime/vector-engine-registry.ts`) — a keyed cache that returns a shared `VectorDatabaseEngine` instance per collection. `RuntimeHints` itself carries no vector-engine accessor; it's only used by the registry to build the embedding client.
