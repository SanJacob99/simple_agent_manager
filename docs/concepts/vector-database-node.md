...
Resolved by `src/utils/graph-to-agent.ts` into `AgentConfig.vectorDatabases: ResolvedVectorDatabaseConfig[]`. The four vector tools live in `server/runtime/vector-tools/` and share a `VectorToolContext` whose `getEngine(label?)` closure calls `getOrCreateVectorEngine(config, label, runtime)` from `server/runtime/vector-engine-registry.ts` to resolve a shared `VectorDatabaseEngine` instance per collection.
