...
- `validateAgentRuntimeGraph()` in `src/utils/graph-to-agent.ts` (951-983) encodes an "exactly one connected Provider Node" rule (`missing_provider` / `duplicate_provider` errors), but that function is not called from any UI or server code path today — only its own test exercises it. In practice, connecting more than one Provider Node is not rejected at runtime; config resolution silently uses whichever one is found first.
