...
3. `buildSystemPrompt()` assembles the final system prompt based on `systemPromptMode`, tool summaries, skills, workspace path, and runtime metadata. The `## Runtime` section emits `Runtime: host=… | os=… | model=…` with **no** `thinking=<level>` field — the thinking level is communicated to the provider via the API `reasoning.effort` parameter, and a plain-text `thinking=<level>` directive in that line can cause Gemini 3 to switch to a "think silently" mode (documented by Google's Gemini 3 prompting guide). A separate `## Reasoning` section still emits one informational prose line, `Thinking effort (provider-side): <level>`, alongside a reasoning-visibility note. See [system-prompt.md](system-prompt.md) for the full mode/section reference.
