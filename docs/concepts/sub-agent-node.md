...
| `recursiveSubAgentsEnabled` | `boolean` | `false` | When true, the sub may call `sessions_spawn` itself, though recursive fan-out is disabled at runtime regardless (see Runtime Behavior #6). The property editor labels this "Off by default. Recursion is disabled until product is ready." |

## Connections

- Receives from: Provider (optional), Tools (REQUIRED), Skills (any), MCP (any)
- Sends to: Agent Node only (peripheral→agent edge)
- Multiple Sub-Agent Nodes may attach to one agent. Names are not validated for uniqueness — if two Sub-Agent Nodes share a `name`, `resolveAgentConfig()` silently drops every node with the duplicated name from the resolved `AgentConfig.subAgents` list (`src/utils/graph-to-agent.ts`), with no error surfaced in the UI
