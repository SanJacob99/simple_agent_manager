...
`src/utils/graph-to-agent.ts` resolves each connected Telemetry node into a `ResolvedTelemetryConfig` entry on `AgentConfig.telemetry` (`shared/agent-config.ts`). The resolver always produces an array — agents without a Telemetry node get `telemetry === []`, not `undefined` — and (once wired) the runtime emits nothing when that array is empty.
