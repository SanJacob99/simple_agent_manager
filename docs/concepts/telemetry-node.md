...
`src/utils/graph-to-agent.ts` resolves each connected Telemetry node into a `ResolvedTelemetryConfig` entry on `AgentConfig.telemetry` (`shared/agent-config.ts`). The list is optional — agents without a Telemetry node have `telemetry === []` (or omitted, for hand-built configs) and the runtime emits nothing.
