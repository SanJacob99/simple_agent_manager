...
In the current implementation, compaction is no longer only an in-memory concern. When the runtime binds an active session transcript, any strategy that actually shrinks the message list — `trim-oldest`, `sliding-window`, or `summary` — writes a real `compaction` entry into the session file through `SessionManager`. That allows resumed sessions to rebuild context from persisted compaction summaries instead of depending on a still-live process.
...
- when a live transcript is bound, any strategy that actually shrinks the messages (not just `summary`) appends a persisted `compaction` entry via `SessionManager.appendCompaction(...)`
