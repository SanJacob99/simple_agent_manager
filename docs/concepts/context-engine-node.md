...
In the current implementation, compaction is no longer only an in-memory concern. When the runtime binds an active session transcript, any compaction strategy that shortens the message array writes a real `compaction` entry into the session file through `SessionManager`. That allows resumed sessions to rebuild context from persisted compaction entries instead of depending on a still-live process.
...
- when a live transcript is bound, **any** strategy that actually shortens the message array (not just `summary`) appends a persisted `compaction` entry via `SessionManager.appendCompaction(...)`
