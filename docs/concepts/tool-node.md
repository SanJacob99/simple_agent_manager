...
> **Deprecated.** Sub-agent capability is now declared via the [Sub-Agent Node](sub-agent-node.md); `maxSubAgents` has no runtime effect. `subAgentSpawning` is a legacy compatibility shim, not fully inert: `server/sessions/session-tools.ts` still honors it as a fallback that enables sub-agent session tools (e.g. `sessions_yield`) for older graphs with no declared Sub-Agent Node, and `RunCoordinator` passes it through to that check (`server/agents/run-coordinator.ts:1556`). Newly authored graphs should rely solely on the Sub-Agent Node.
...
- every built-in tool, including `calculator` and `web_fetch`, ships as a self-describing `ToolModule` under `server/tools/builtins/` (see [adding-a-tool.md](./adding-a-tool.md)); none are placeholders/stubs
