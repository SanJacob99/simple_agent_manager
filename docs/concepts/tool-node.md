...
> **Deprecated.** Sub-agent capability is now declared via the [Sub-Agent Node](sub-agent-node.md). `maxSubAgents` is no longer used by the runtime. `subAgentSpawning` is honored only as a back-compat shim (`server/sessions/session-tools.ts`): on a graph with no Sub-Agent nodes attached, it can still enable the `sessions_yield`/`subagents` tool surface, though `sessions_spawn` itself requires at least one declared Sub-Agent node regardless of this flag. Newly authored graphs should not rely on it — attach a Sub-Agent node instead.
