...
> **Note.** `send_message` is listed in the `communication` group but is **not** in `IMPLEMENTED_TOOL_NAMES`. It will be silently filtered from the system prompt tool advertisement unless it has been added to the live tool catalog via `GET /api/tools`. Enabling the `messaging` profile or `communication` group includes `text_to_speech` (implemented) but omits `send_message`.
