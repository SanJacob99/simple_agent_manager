...
> **Deprecated.** Sub-agent capability is now declared via the [Sub-Agent Node](sub-agent-node.md), and `maxSubAgents` has no runtime effect. `subAgentSpawning` is not fully dead, though: `server/sessions/session-tools.ts` (`createSessionTools`) still honors it as a back-compat shim — if it is `true`, sub-agent tools like `sessions_yield` are registered even when the agent has no connected `SubAgentNode` (`sessions_spawn` still requires at least one declared sub-agent to know what names to expose). Existing graphs continue to load; new graphs should rely on the Sub-Agent Node rather than this flag.
