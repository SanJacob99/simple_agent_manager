...
| `toolSettings.exec.skill` | `string` | `""` | Optional inline markdown override for the exec skill |
| `toolSettings.codeExecution.apiKey` | `string` | `""` | xAI API key. Empty reads `XAI_API_KEY` from env |
...
| `toolSettings.webSearch.tavilyApiKey` | `string` | `""` | Tavily API key. Empty reads `TAVILY_API_KEY` from env; no key falls back to DuckDuckGo |
