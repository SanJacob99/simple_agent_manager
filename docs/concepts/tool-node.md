...
> **Deprecated.** Sub-agent capability is now declared via the [Sub-Agent Node](sub-agent-node.md). `maxSubAgents` is no longer used by the runtime. `subAgentSpawning` is still read as a back-compat shim in `server/sessions/session-tools.ts`: when `true`, it enables the `sessions_yield`/`subagents` session tools even if no Sub-Agent Node is declared (`hasDeclaredSubAgents || ctx.subAgentSpawning`). New graphs should express sub-agent capability with a Sub-Agent Node instead of this flag.
