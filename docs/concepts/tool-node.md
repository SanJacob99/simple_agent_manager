...
> **Deprecated.** Sub-agent capability is now declared via the [Sub-Agent Node](sub-agent-node.md). `maxSubAgents` is no longer used by the runtime. `subAgentSpawning` is honored only as a legacy compatibility shim (`server/sessions/session-tools.ts`): for graphs with no declared Sub-Agent Nodes, it can still enable the `sessions_yield` and `subagents` tools, though `sessions_spawn` itself always requires actual Sub-Agent Node declarations regardless of this flag. Existing graphs continue to load either way.
