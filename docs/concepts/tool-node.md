...
> **Deprecated.** Sub-agent capability is now declared via the [Sub-Agent Node](sub-agent-node.md). `maxSubAgents` is no longer used by the runtime. `subAgentSpawning`, however, still has effect as a back-compat shim: `server/agents/run-coordinator.ts` forwards it into `SessionToolContext`, and `server/sessions/session-tools.ts` OR's it with "has declared Sub-Agent nodes" to decide whether the `sessions_yield`/`subagents` tools are exposed — so an old graph with no Sub-Agent nodes but `subAgentSpawning: true` still gets those tools. New graphs should use the Sub-Agent Node instead of relying on this flag.
