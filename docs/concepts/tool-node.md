...
- `calculator` is the only tool still served from the legacy `TOOL_CREATORS` fallback; every other tool (including `web_fetch`) is a self-describing `ToolModule` served through the registry, same as `read_file` or `browser`
