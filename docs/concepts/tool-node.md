...
> **Deprecated.** Sub-agent capability is now declared via the [Sub-Agent Node](sub-agent-node.md). `maxSubAgents` is no longer used by the runtime and has no effect. `subAgentSpawning` is still read as a compatibility fallback, though: `server/sessions/session-tools.ts` enables the sub-agent tool surface when `hasDeclaredSubAgents || ctx.subAgentSpawning` is true, and `server/agents/run-coordinator.ts` still threads `tools.subAgentSpawning` into that check. Existing graphs continue to load either way.
