...
> **Deprecated.** Sub-agent capability is now declared via the [Sub-Agent Node](sub-agent-node.md). `maxSubAgents` is no longer read anywhere. `subAgentSpawning` still has an effect for backward compatibility: `server/sessions/session-tools.ts` enables the sub-agent tool surface when either a Sub-Agent Node is connected *or* `subAgentSpawning` is `true`, so old graphs saved before the Sub-Agent Node existed keep working without edits. Prefer the Sub-Agent Node for new graphs.
