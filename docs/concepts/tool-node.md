...
> **Legacy.** Sub-agent capability is now declared via the [Sub-Agent Node](sub-agent-node.md). `subAgentSpawning` is honored as a back-compat shim in `server/sessions/session-tools.ts` for older graphs that still set it; it enables `sessions_yield` and `subagents` tools but **not** `sessions_spawn` (which requires at least one declared Sub-Agent Node). `maxSubAgents` is preserved in the config but is currently unused at runtime. New graphs should use Sub-Agent Nodes instead of these flags.
