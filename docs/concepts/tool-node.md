...
1. Determine the active groups: if `enabledGroups` is non-empty, use them directly; otherwise fall back to the groups defined by the selected `profile`. The two sources are mutually exclusive — `enabledGroups` **replaces** the profile's group set rather than adding to it.
...
5. Deduplicate and canonicalize aliases (e.g. `bash` → `exec`, `code_interpreter` → `code_execution`)
