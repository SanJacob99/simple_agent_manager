...
- `server/sam-agent/sam-agent-tools.ts` — registers the four SAMAgent tools (see Tool Surface below) using the same `AgentTool` interface that `server/tools/tool-factory.ts` produces. The tools are **not** registered in the global tool registry — they are passed directly to `AgentRuntime` for SAMAgent only.
...
`AgentRuntime` itself is graph-agnostic (the coupling is in `RunCoordinator`'s setup and in the tool factory needing an agent config), so it can be reused as-is.
...
- `samAgent:prompt { text, currentGraph }` — user sends a turn. `currentGraph` is `{ nodes: [{id, type, data}], edges: [{id, source, target}] }` serialized client-side.
- `samAgent:abort` — user cancels in-flight turn.
- `samAgent:event` — server-to-client streaming of `text_delta`, `tool_call`, `tool_result`, `done`, `error`. Same shape as the existing `agent:event` stream so the UI can reuse rendering primitives.
...
When the user sends a prompt, the client serializes the current graph (a compact projection: id, type, data only — not React Flow positions) and attaches it to the WS message. The server injects it into the system prompt for that turn under a `<current_graph>` block. The model always knows what's on the canvas without spending a tool call.
...
All four tools execute server-side. Only `propose_workflow_patch` produces UI side-effects.
...
- For each agent touched (newly added or updated), the resulting graph must resolve to a valid `AgentConfig` via `shared/resolve-tool-names.ts` and the same checks `resolveAgentConfig()` performs for runtime. (Re-using existing resolver code, not re-implementing.)
...
3. **Connection rules** — peripherals connect to `agent` or `subAgent` only; sub-agent peripherals limited to `tools`, `provider`, `skills`, `mcp`; storage + contextEngine required for interactive chat.
...
6. **Doc-reading guidance** — call `read_doc` for specifics before authoring patches that touch unfamiliar node types.
...
- **Header**
  - "SAMAgent" title.
  - Model picker (small dropdown) reading from `useModelCatalogStore`, filtered to providers with configured keys in `useSettingsStore`. Default selection priority: previously chosen `samAgent.modelId` from settings, else first Anthropic model available, else first model in the catalog.
...
- `src/runtime/sam-agent-client.ts` (new) — small client wrapping `agentClient.ts` for the `samAgent:*` namespace.
...
- **Apply-card state** — encoded into the transcript message that holds the tool result, so a reload reconstructs whether each card is pending / applied / discarded.
- **Selected model** — stored in `useSettingsStore` under a new `samAgent.modelId` field, persisted via the existing settings server route.
...
3. `AgentRuntime.run()` streams events: `text_delta`, `tool_call`, `tool_result`, `done`.
...
6. Server appends user + assistant turns to JSONL on `done`.
7. On Apply click: `graphStore.applyPatch(patch)` runs locally; apply state is persisted into the assistant message's stored tool result.
...
src/settings/                           # + samAgent.modelId field + UI
...
7. Settings: `samAgent.modelId` + provider gate.
