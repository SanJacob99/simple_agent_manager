...
| `mcp` | Attaches an agent to a Model Context Protocol server (local subprocess or remote HTTP/SSE) so it can call its tools |
...
| `evals` | Attaches a dataset of input → expected cases to an agent and scores its replies with deterministic graders or an LLM judge |
...
| `cron` | Attaches a recurring, time-triggered run (standard cron schedule) to an agent, with its own prompt and per-tick timeout |
