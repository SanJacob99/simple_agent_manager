...
- Several node types and tool names are still extension surfaces rather than fully wired product features. In particular, `connectors`, `vectorDatabase`, `cron`, and `mcp` need runtime inspection before you treat them as end-to-end features — see their concept docs in `docs/concepts/` for exactly what is and isn't wired. Peer-to-peer agent comms (`agentComm`) is wired in v1 with safety and loop controls (turn/depth/token limits, rate limiting, channel-session isolation).
- Nearly every tool listed in `shared/resolve-tool-names.ts`'s `ALL_TOOL_NAMES` has a real implementation under `server/tools/builtins/` (exec, code execution, calculator, web search/fetch, browser, filesystem tools, image, canva, text-to-speech, music generation, human-in-the-loop prompts, memory tools, session tools). Verify `send_message` (`agentComm`) before treating it as fully wired end-to-end.
...
| `mcp` | Configuration surface for arbitrary MCP servers (schema-only — no MCP client runs server-side yet) |
| `provider` | Attaches a model provider plugin (e.g. OpenRouter) with its auth method and credentials |
| `subAgent` | Delegates a scoped sub-task to a synthetic child agent with an overridable config subset |
| `guardrails` | Input/output content rules (max length, blocked terms, PII patterns) with a warn/block action |
...
| `evals` | Test-case suite (contains/regex/json_schema/llm_judge/custom graders) scored against a pass threshold |
...
| `cron` | Time-triggered agent runs on a configurable schedule; the scheduler and dispatch path are wired, though `sessionMode`/`retentionDays` are not yet (see `docs/concepts/cron-node.md`) |
