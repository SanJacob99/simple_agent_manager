...
| `a2a` | Agent-to-Agent (A2A) interop: publishes an Agent Card and accepts remote tasks, and/or registers remote A2A agents as callable delegates — cross-framework agent interop, the way MCP standardized tools |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `a2a` ship with node UI, resolved config, and a unit-tested engine, but the run-coordinator / HTTP-surface wiring is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
