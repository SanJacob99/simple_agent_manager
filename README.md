...
| `a2a` | Agent-to-Agent (A2A) interop: publish an agent card and accept remote tasks (server) and/or register remote A2A agents as callable `a2a_<name>` delegate tools (client), over JSON-RPC / gRPC / REST |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `a2a` ship with node UI, resolved config, and a unit-tested engine, but the run-coordinator / route wiring is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
