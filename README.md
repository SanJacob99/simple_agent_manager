...
| `a2a` | Agent-to-Agent interop: publish an Agent Card so remote clients can discover and task this agent (server), and/or register remote A2A agents as callable delegate tools (client), over JSON-RPC / gRPC / REST |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `a2a` ship with node UI, resolved config, and a unit-tested engine, but the runtime/transport wiring is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
