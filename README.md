...
| `sandbox` | Execution safety for code-running tools: an isolation backend (none / workdir / container / microVM), per-command resource ceilings (CPU / memory / wall-clock / process count), a filesystem scope, and a network egress policy (none / allowlist / full), with a block / warn violation policy |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `sandbox` ship with node UI, resolved config, and a unit-tested engine, but the run-coordinator / tool-layer wiring is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
