...
| `a2a` | Agent-to-Agent interop: publish an agent card and accept remote tasks (server) and/or register remote A2A agents as callable delegates (client), for cross-framework agent interoperability |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `a2a` ship with node UI, resolved config, and a unit-tested engine, but the runtime wiring (run-coordinator for the first four, an `server/a2a/` HTTP surface for A2A) is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
