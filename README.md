...
| `trigger` | Event-driven runs beyond cron: webhook, file-watch, queue, or inbound-email sources, each with a payload filter, debounce, and concurrency ceiling; the prompt is rendered from the event payload |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `trigger` ship with node UI, resolved config, and a unit-tested engine, but the run-coordinator / transport wiring is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
