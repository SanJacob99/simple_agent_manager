...
| `sandbox` | Execution safety for code-running tools: an isolation backend (local / container / microVM / gVisor), CPU / memory / wall-clock / process ceilings, a network egress policy (none / allowlist / all), and a filesystem scope, with a block / warn / terminate violation policy |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `sandbox` ship with node UI, resolved config, and a unit-tested engine, but the run-coordinator wiring is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
