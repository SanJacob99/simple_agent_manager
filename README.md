...
| `a2a` | Agent-to-Agent (A2A) interop: exposes the agent as an A2A server (publishes an agent card, accepts remote tasks) and/or registers remote A2A agents as callable `a2a_send_*` delegates — the emerging cross-framework interop protocol, much as MCP standardized tools |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `a2a` ship with node UI, resolved config, and a unit-tested engine, but the runtime wiring (run-coordinator finalize/turn loop; for `a2a`, the server card route and remote-delegate tools) is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
