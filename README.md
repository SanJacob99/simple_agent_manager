...
| `a2a` | Agent-to-Agent (A2A) interop: publishes an agent card and accepts inbound tasks (server) and/or registers remote A2A agents as callable delegates (client), for cross-framework agent interop |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `a2a` ship with node UI, resolved config, and a unit-tested engine, but the runtime wiring (run-coordinator for the first four, a `server/a2a/` router for `a2a`) is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
