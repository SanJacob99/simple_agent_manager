...
| `mcp` | Attaches an agent to a Model Context Protocol server (local subprocess or remote HTTP/SSE) |
| `provider` | Model provider identity, auth, and base URL resolution for the connected agent |
| `subAgent` | Declares a named, one-shot sub-agent the parent agent can dispatch via `sessions_spawn` |
| `guardrails` | Input/output safety checks that block or warn before/after a model turn |
...
| `cron` | Schedules recurring agent runs on a cron expression, with persistent/ephemeral session modes |
