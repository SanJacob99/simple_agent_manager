...
| `a2a` | Agent-to-Agent interop: exposes the agent as an A2A server (publishes an Agent Card, accepts remote tasks) and/or registers remote A2A agents as callable delegates, for cross-framework interop beyond the in-process `agentComm` bus |
