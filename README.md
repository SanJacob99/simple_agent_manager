...
- Several node types and tool names are still extension surfaces rather than fully wired product features. In particular, `connectors`, `vectorDatabase`, `cron`, and `mcp` need runtime inspection before you treat them as end-to-end features — the MCP client itself (subprocess spawn, HTTP/SSE transport, `mcp:status` emission) is not yet implemented, so `connectors` and `mcp` both resolve into `AgentConfig.mcps[]` but nothing consumes that array at runtime yet. Peer-to-peer agent comms (`agentComm`) is wired in v1 with safety and loop controls (turn/depth/token limits, rate limiting, channel-session isolation).
...
| `connectors` | Curated external integrations (e.g. GitHub) that resolve into an MCP server |
...
| `mcp` | Attaches an agent to an arbitrary Model Context Protocol server (stdio or HTTP/SSE) |
| `provider` | Declares which provider plugin an agent uses and how auth/base URL are resolved |
| `subAgent` | Declares a named, one-shot sub-agent the parent agent can dispatch via `sessions_spawn` |
| `guardrails` | Input/output safety checks that block or warn before/after a turn reaches the model |
...
| `cron` | Time-triggered agent runs on a configurable schedule, dispatched independently of the chat drawer |
