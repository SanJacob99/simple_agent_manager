...
| `a2a` | Agent-to-Agent interop: exposes this agent as an A2A server (publishes an agent card, accepts remote tasks) and/or registers remote A2A agents as callable delegates via a `delegate_to_agent` tool, with per-remote auth and delegation-depth control |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `a2a` ship with node UI, resolved config, and a unit-tested engine, but the run-coordinator / server-transport wiring is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
