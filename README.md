...
| `a2a` | Agent-to-Agent (A2A) interop: publish an Agent Card so other frameworks can task this agent, register remote A2A agents as callable delegates, or both — with advertised skills, task/message envelopes, and a fail / warn / ignore remote-error policy |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `a2a` ship with node UI, resolved config, and a unit-tested engine, but the run-coordinator (and, for `a2a`, the HTTP server) wiring is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
