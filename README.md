...
| `a2a` | Agent-to-Agent interop: exposes the agent as an A2A server (agent card + task endpoint) and/or registers remote A2A agents as callable delegates, with capability/auth advertisement and a task state machine |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `a2a` ship with node UI, resolved config, and a unit-tested engine, but the runtime wiring (run-coordinator finalize/turn loop, or the `server/a2a/` router for `a2a`) is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
