...
| `sandbox` | Execution environment for code-running tools: isolation level, CPU/memory/wall-clock ceilings, network-egress policy, filesystem mount scope, and command allow/deny lists, with a warn / block violation policy |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `sandbox` ship with node UI, resolved config, and a unit-tested engine, but the run-coordinator / tool-factory wiring is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
