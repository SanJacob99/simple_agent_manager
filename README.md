...
| `a2a` | Agent-to-Agent (A2A) interop: publish this agent as an A2A server (agent card + inbound tasks) and/or register remote A2A agents as callable delegates, over the cross-framework A2A protocol |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `a2a` ship with node UI, resolved config, and a unit-tested engine, but the run-coordinator wiring is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
