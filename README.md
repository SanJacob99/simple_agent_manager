...
| `a2a` | Agent-to-Agent interop: publish an agent card and accept remote tasks (server) and/or register remote A2A agents as callable delegates (client), across JSON-RPC / gRPC / HTTP+JSON transports |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `a2a` ship with node UI, resolved config, and a unit-tested engine, but the run-coordinator / network wiring is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
