...
| `a2a` | Agent-to-Agent interop: exposes the agent as an A2A server (publishes an agent card, accepts remote task/message envelopes) and/or registers remote A2A agents as callable delegates, with per-agent auth, timeout, and a fail / warn / ignore remote-error policy |
...
> **Scaffolding note:** `telemetry`, `structuredOutput`, `budget`, `reflection`, and `a2a` ship with node UI, resolved config, and a unit-tested engine, but the server/run-coordinator wiring is still pending — treat them as extension surfaces until that path is verified end-to-end. See `docs/roadmap/2026-modernization.md`.
