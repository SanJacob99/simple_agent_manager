...
## 2024-05-24 - SSRF via DNS Rebinding in Built-in Tools
**Vulnerability:** The `web_fetch`, `show_image`, and `image_analyze` tools were vulnerable to Server-Side Request Forgery (SSRF) and DNS rebinding attacks. They lacked sufficient validation of URLs, allowing external agents to query internal services (e.g. localhost, 169.254.169.254, or private IP ranges).
**Learning:** Checking for internal IPs before making a request is insufficient because an attacker can return a safe IP for the first check and a private IP for the actual request (DNS rebinding / TOCTOU). Full TOCTOU protection requires a custom HTTP Agent/dispatcher (e.g., undici) to pin the validated IP at the socket level. Furthermore, default `fetch` follows redirects, bypassing initial URL validation entirely.
**Prevention:** Always use the shared `validateSafeUrl` utility from `server/tools/builtins/web/url-validator.ts` when making HTTP requests via tool implementations. Disable automatic redirect following (`redirect: 'error'` or handle manually) and pass the `dispatcher` to `fetch` to pin the validated IP.
