...
## 2026-05-08 - [CRITICAL] Fix path traversal in music-generate
**Vulnerability:** The `music_generate` tool improperly resolved the output path using `path.resolve` without checking if the resulting path escaped the intended workspace boundary. An attacker could potentially supply a `filename` hint (e.g. `../../../../etc/passwd`) allowing them to write files to arbitrary locations on the file system.
**Learning:** `path.resolve` combined with user-provided hints natively processes `..` navigation segments. Even if the base directory was resolved correctly, the combination of user input and `path.resolve` enables arbitrary path escapes.
**Prevention:** Whenever building file paths with user input, especially using `path.resolve` or `path.join`, always append a suffix validation to confirm that the `resolvedTarget.startsWith(resolvedBase + path.sep)` and handle root matches.
