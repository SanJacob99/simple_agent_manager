...
## 2024-05-18 - SSRF Vulnerability via Missing URL Validation in Image Fetch Tools
**Vulnerability:** The `show_image` and `image` (analyze) tools lacked Server-Side Request Forgery (SSRF) protections. While `web_fetch` had robust SSRF mitigation (validating protocols, blocking local/internal hostnames, and resolving DNS to block internal IP ranges), these image tools directly passed user-provided URLs to `fetch`, allowing potential access to internal network resources.
**Learning:** Security validations for common operations like external HTTP requests can easily become fragmented when implemented inline rather than in shared utilities. The robust SSRF protections developed for the general web fetcher were not propagated to newer or specialized tools that also perform fetching.
**Prevention:** Always extract critical security validations (like SSRF prevention, input sanitization, and path traversal checks) into shared, explicitly named utility functions (`url-validator.ts`, `validateSafeUrl`). Ensure all modules performing external network I/O import and execute this shared validation to maintain a consistent security posture across the codebase.
