...
## 2026-05-18 - [CRITICAL] Fix Path Traversal in Music Generate Tool
...
## 2026-06-25 - [CRITICAL] Fix absolute path bypass in browser tool path traversal prevention
**Vulnerability:** The browser tool's `resolveUserDataDir` function attempted to handle absolute `userDataDir` inputs via `path.isAbsolute(input) ? input : path.resolve(base, input)`, and then applied a `startsWith` check against `base`. However, this meant an attacker could supply an absolute path containing directory traversal sequences (e.g., `/base/../etc/passwd`). `path.isAbsolute` would return true, and the unnormalized path would pass the `startsWith("/base")` check while actually resolving to `/etc/passwd`.
**Learning:** Checking boundary containment (`startsWith`) on unnormalized paths allows bypasses if the path contains `../` sequences that logically escape the boundary but textually match the prefix. Additionally, preserving `path.isAbsolute` for user-supplied paths in sandboxed contexts entirely defeats the sandbox.
**Prevention:** Always resolve all user inputs against the base directory (`path.resolve(base, input)`) without conditionally branching on `isAbsolute`, ensuring the resulting path is fully normalized before applying boundary checks (e.g., `!resolved.startsWith(base + path.sep)`).
