...
## 2023-10-24 - SSRF Vulnerability in Image Loading Tools
**Vulnerability:** The tools `show_image` and `image_analyze` used native `fetch()` without any validation to load remote images. This could be exploited as a Server-Side Request Forgery (SSRF) vulnerability if an attacker supplied an internal URL or a domain configured for DNS rebinding.
**Learning:** `fetch()` does not automatically block local/private IP ranges or protect against DNS rebinding. While `web_fetch` had a manual mitigation for this, the image processing tools were overlooked.
**Prevention:** Always validate external URLs and pin resolved safe IP addresses using a custom Undici agent when performing backend HTTP requests. Shared this logic through `url-validator.ts` as `fetchSafeUrl` for reuse across the codebase to ensure all outgoing fetches are SSRF-protected.
