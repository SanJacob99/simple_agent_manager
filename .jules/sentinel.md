...
## 2026-06-29 - [CRITICAL] Fix Path Traversal Bypass in StorageEngine via Absolute Paths
**Vulnerability:** The `StorageEngine.resolveTranscriptPath` method bypassed sandboxed directory constraints if `path.isAbsolute(entry.sessionFile)` evaluated to true, allowing agents or users to overwrite/read arbitrary absolute paths like `/etc/passwd`.
**Learning:** Checking `path.isAbsolute()` and returning the path directly bypasses sandbox resolution controls (`_safeJoin`). Absolute paths must still be verified to ensure they resolve within the intended sandbox boundary, just like relative paths.
**Prevention:** Never conditionally bypass boundary checking for absolute paths in sandboxing code. Always pass them through boundary enforcement logic (e.g., `_safeJoin`) to confirm the absolute path still begins with the safe directory prefix.
