...
## 2026-05-18 - [CRITICAL] Fix Path Traversal via Absolute Paths in StorageEngine
**Vulnerability:** The `StorageEngine.resolveTranscriptPath` method explicitly checked if a provided `sessionFile` was an absolute path (`path.isAbsolute(entry.sessionFile)`) and if so, returned it directly without applying the path traversal verification inside `_safeJoin`. This allowed agents to read/write arbitrary files across the system by providing an absolute path as the `sessionFile`.
**Learning:** Hardcoding a bypass for absolute paths in boundary-checking routines completely defeats the sandboxing purpose. Sandboxes must ensure that *all* paths, absolute or relative, strictly reside inside the designated root.
**Prevention:** Never skip boundary validation (`startsWith(baseDir + path.sep)`) for absolute paths. Let the safe resolution logic evaluate all paths and reject them if they fall outside the allowed directory.
