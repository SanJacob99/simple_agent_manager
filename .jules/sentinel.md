...
## 2026-05-20 - [CRITICAL] Fix absolute path bypass in sandboxed file system operations
**Vulnerability:** In `server/storage/storage-engine.ts`, the path traversal prevention logic specifically bypassed the safe join check if the user-provided path was absolute (`path.isAbsolute(entry.sessionFile)`). This allowed an attacker to easily break out of the directory sandbox and read/write to arbitrary paths like `/etc/passwd`.
**Learning:** Checking if a path is absolute before performing boundary checks creates a severe path traversal vulnerability. Absolute paths must also be resolved and checked to ensure they reside strictly within the intended base directory.
**Prevention:** Never conditionally skip sandbox validation checks based on the format of the path (e.g., `path.isAbsolute`). Always resolve paths and strictly verify the result starts with the base directory and separator.
