...
## 2026-05-19 - [CRITICAL] Fix Path Traversal via Absolute Path Bypass in StorageEngine
**Vulnerability:** The `StorageEngine`'s `resolveTranscriptPath` method intentionally skipped the `_safeJoin` validation if the user-provided `sessionFile` was an absolute path (via `path.isAbsolute`). This allowed an attacker to bypass directory boundary checks entirely by supplying absolute paths like `/etc/passwd`.
**Learning:** Checking if a path is absolute does not guarantee safety. An absolute path is still subject to reading or writing outside the sandboxed base directory if it isn't explicitly validated against that intended boundary.
**Prevention:** Never skip boundary validations for absolute paths. Pass both relative and absolute user inputs through the same validation logic (e.g., `_safeJoin`) to ensure they securely resolve strictly inside the intended base directory.
