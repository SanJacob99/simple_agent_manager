...
## 2026-06-30 - [CRITICAL] Fix Path Traversal via Absolute Paths in StorageEngine
**Vulnerability:** The StorageEngine skipped `_safeJoin` validation when resolving `sessionFile` if the user-provided path was an absolute path (`path.isAbsolute`). This allowed bypassing the sandboxed directory checks and gave agents the ability to read and write arbitrary files anywhere on the system (e.g., `/etc/passwd`).
**Learning:** Checking for `path.isAbsolute` and returning it directly circumvents all bounds checking. `path.resolve` handles absolute paths natively by resolving them to the root and replacing the base path entirely, which means a safe join utility can still perform boundary validation on the resulting absolute path.
**Prevention:** Never skip boundary validation (e.g., `_safeJoin`) for absolute paths. Pass absolute paths through the same safe joining function to ensure they resolve within the intended base directory.
