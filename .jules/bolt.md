...
## 2024-05-19 - Avoid Chained Array Methods (Split/Map/Filter) in Hot Paths
**Learning:** Chained array methods like `.split().filter().map()` create intermediate arrays that cause memory churn and GC pauses in high-frequency paths. This causes significant performance bottlenecks.
**Action:** Use single-pass for loops instead of chained array methods for critical text extraction code paths.
