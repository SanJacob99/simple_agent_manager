...
## 2024-06-03 - O(N^2) I/O Bottleneck in Eviction Loops
**Learning:** Calling aggregate directory size functions (like `getDiskUsage()`) repeatedly inside an eviction loop creates severe O(N^2) I/O bottlenecks and breaks simulation (dry run) modes because the physical files aren't deleted.
**Action:** To track running size totals during batch deletions, use `fs.stat` on the individual file being deleted and subtract its size from the running total, rather than re-scanning the entire directory.
