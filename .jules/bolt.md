...
## 2025-08-05 - Avoid O(N^2) filesystem IO in eviction loop
**Learning:** Calling an aggregated directory size function (like `getDiskUsage()`) inside an eviction loop causes an O(N^2) bottleneck. Additionally, predicting free space via `fs.stat()` before deletion can be brittle if deletion fails.
**Action:** Let deletion operations track and return the exact number of bytes successfully freed, and subtract that from a running total.
