...
## 2024-05-19 - O(N^2) File System I/O Bottleneck in Eviction Loops
**Learning:** Calling an aggregate directory size function like `getDiskUsage()` repeatedly inside a loop that deletes files creates an O(N^2) I/O bottleneck. Furthermore, since `getDiskUsage()` reads actual disk state, it prevents simulation (dry run) modes from correctly reflecting the theoretical size reduction, as files aren't actually deleted during a dry run.
**Action:** When enforcing disk budgets or removing files in a loop, fetch the size of the individual file being processed using `fs.stat` and subtract it from a running total (`currentUsage`) instead of recalculating the entire directory size on each iteration.
