...
**Learning:** Repeatedly calling aggregate directory size functions like `getDiskUsage()` inside eviction loops causes severe O(N^2) file system I/O overhead.
**Action:** Instead of re-calculating the entire directory size, have the deletion functions return the exact number of bytes successfully freed and subtract it from a running total.
