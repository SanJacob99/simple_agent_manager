...
## 2024-05-19 - Efficient JSONL Parsing
**Learning:** Parsing large JSONL files using `.split('\n').filter(...).map(...)` allocates massive intermediate arrays, causing significant memory churn and potential GC pauses.
**Action:** Use a single-pass extraction loop with `indexOf('\n')` and `substring()` to construct the final array directly without intermediate allocations.
