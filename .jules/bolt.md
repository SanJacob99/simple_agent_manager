...
## 2024-05-19 - Avoid O(N^2) I/O in eviction loops
**Learning:** Calling an aggregate directory size function like `getDiskUsage()` repeatedly inside an eviction loop creates an O(N^2) file system I/O bottleneck. Furthermore, calling it in a loop doesn't update the size properly during simulation (dry run) modes because no files are actually deleted, causing infinite or excessive loops.
**Action:** Use `fs.stat` to get the size of the individual file being considered for deletion and subtract it from the running total, instead of recalculating the entire directory size.
