...
/**
 * Validates a URL to prevent SSRF and DNS rebinding attacks.
 * Enforces http/https protocols and performs DNS resolution against internal/loopback ranges.
 * Returns an undici Agent (dispatcher) pinned to the validated safe IP address.
 */
export async function validateSafeUrl(urlStr: string): Promise<{ safeUrl: string, dispatcher: Agent }> {
  ...
  return { safeUrl: urlStr, dispatcher };
}
