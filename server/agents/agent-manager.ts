...
export class AgentManager {
  ...
  async restoreFromDisk(storagePath: string): Promise<number> {
    ...
    try {
      ...
      // ⚡ Bolt Optimization: Use Promise.all with .map() to read configs and start agents
      // concurrently instead of sequentially to eliminate N+1 I/O latency overhead on boot.
      await Promise.all(
        entries
