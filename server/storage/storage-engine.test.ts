...
describe('StorageEngine', () => {
  ...
  describe('transcript path resolution', () => {
    ...
    it('uses sessionFile when explicitly set', () => {
      // Must be an absolute path *inside* the agentDir to be valid now, or a relative path
      // that resolves inside it. Here we use an absolute path correctly bounded.
      const boundedAbsPath = path.resolve(engine['agentDir'], 'transcript.jsonl');
      const result = engine.resolveTranscriptPath(makeEntry({ sessionFile: boundedAbsPath }));
      expect(result).toBe(boundedAbsPath);
    });
    ...
    it('throws error when sessionFile explicitly set to absolute path traversing out', () => {
      expect(() => {
        engine.resolveTranscriptPath(makeEntry({ sessionFile: '/etc/passwd' }));
      }).toThrow('Path traversal detected');
    });
