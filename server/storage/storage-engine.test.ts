...
describe('StorageEngine', () => {
  ...
  describe('transcript path resolution', () => {
    ...
    it('uses sessionFile when explicitly set to an absolute path inside agentDir', () => {
      const absolutePath = path.resolve(engine.getAgentDir(), 'custom/path/transcript.jsonl');
      const result = engine.resolveTranscriptPath(makeEntry({ sessionFile: absolutePath }));
      expect(result).toBe(absolutePath);
    });

    it('throws error when sessionFile explicitly set to relative path traversing out', () => {
      expect(() => {
