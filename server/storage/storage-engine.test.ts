...
describe('StorageEngine', () => {
  ...
  describe('transcript path resolution', () => {
    ...
    it('uses sessionFile when explicitly set', () => {
      // Must be an absolute path within the agent's base directory
      const safeAbsPath = path.join(engine['agentDir'], 'transcript.jsonl');
      const result = engine.resolveTranscriptPath(makeEntry({ sessionFile: safeAbsPath }));
      expect(result).toBe(safeAbsPath);
    });
    ...
    it('throws error when sessionFile explicitly set to an absolute path traversing out', () => {
      expect(() => {
        engine.resolveTranscriptPath(makeEntry({ sessionFile: '/etc/passwd' }));
      }).toThrow('Path traversal detected');
    });
