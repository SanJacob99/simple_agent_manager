...
describe('StorageEngine', () => {
  ...
  describe('transcript path resolution', () => {
    ...
    it('uses sessionFile when explicitly set (relative to agentDir)', () => {
      const result = engine.resolveTranscriptPath(makeEntry({ sessionFile: 'custom-transcript.jsonl' }));
      expect(result).toContain('custom-transcript.jsonl');
    });

    it('throws error when sessionFile is an absolute path bypassing containment', () => {
      expect(() => {
