...
export class StorageEngine {
  ...
  async enforceDiskBudget(maxBytes: number, highWaterBytes: number, dryRun: boolean): Promise<string[]> {
    ...
    if (currentUsage > highWaterBytes) {
      ...
      for (let i = sessions.length - 1; i >= 0; i--) {
        ...
        if (!dryRun) {
          // ⚡ Bolt Optimization: Subtract exactly freed bytes instead of O(N^2) getDiskUsage() calls
          ...
          try {
            ...
            if (transcriptPath.startsWith(this.sessionsDir + path.sep) || transcriptPath === this.sessionsDir) {
              currentUsage -= stat.isFile() ? stat.size : 0;
            }
          } catch {
            // Ignore missing files in dryRun
          }
        }
      }
    }
  ...
  private async deleteTranscriptFile(entry: Pick<SessionStoreEntry, 'sessionId' | 'sessionFile'>): Promise<number> {
    ...
    try {
      ...
      // Only count bytes if file is within sessionsDir, as getDiskUsage() only tracks that directory
      if (transcriptPath.startsWith(this.sessionsDir + path.sep) || transcriptPath === this.sessionsDir) {
        return stat.isFile() ? stat.size : 0;
      }
      return 0;
    } catch {
      // Ignore missing files or deletion failures so metadata cleanup stays idempotent.
      return 0;
    }
  }
}
