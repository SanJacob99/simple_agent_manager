    ...
    if (currentUsage > highWaterBytes) {
      ...
      for (let i = sessions.length - 1; i >= 0; i--) {
        ...
        if (!dryRun) {
          ...
          // In dryRun, currentUsage wouldn't actually decrease since files aren't deleted.
          // To simulate, we would need to stat the file without deleting it.
          // For simplicity, simulate by subtracting its size.
          const transcriptPath = this.resolveTranscriptPath(session);
          if (transcriptPath.startsWith(this.sessionsDir + path.sep)) {
            try {
              const stat = await fs.stat(transcriptPath);
              if (stat.isFile()) currentUsage -= stat.size;
            } catch {
              // Ignore
            }
          }
