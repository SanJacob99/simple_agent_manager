  ...
  async enforceDiskBudget(maxBytes: number, highWaterBytes: number, dryRun: boolean): Promise<string[]> {
    ...
    if (currentUsage > highWaterBytes) {
      ...
      for (let i = sessions.length - 1; i >= 0; i--) {
        ...
        let freedBytes = 0;
        if (!dryRun) {
          freedBytes = await this.deleteSession(session.sessionKey);
        } else {
          const transcriptPath = this.resolveTranscriptPath(session);
          try {
            const stat = await fs.stat(transcriptPath);
            if (stat.isFile() && transcriptPath.startsWith(this.sessionsDir + path.sep)) {
              freedBytes = stat.size;
            }
          } catch {
            // Ignore missing files during dry-run simulation
          }
        }

        // ⚡ Bolt Optimization: Subtract exactly the space freed instead of
        // calling getDiskUsage() on every iteration, which is O(N^2) I/O.
